...
const xai = new OpenAI({
    ...
    baseURL: typeof window !== "undefined" ? `${window.location.origin}/xai-api/v1` : "http://localhost:3000/xai-api/v1",
    ...
    try {
        const response = await xai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "You are a helpful data-extraction assistant. You must extract and output data strictly in JSON format. Do not return any conversational text."
                },
                {
                    role: "user",
                    content: prompt,
